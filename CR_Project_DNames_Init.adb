...
with CR_Project_DNames_CB; use CR_Project_DNames_CB;
...
package body CR_Project_DNames_Init is
   ...
   procedure Gtk_New(MainDialog : out DNames_Access) is
   begin
      MainDialog := new DNames_Record;
      CR_Project_DNames_Init.Init (MainDialog);
   end Gtk_New;

   procedure Init(MainDialog : access DNames_Record'Class) is
      pragma Suppress (All_Checks);
      Pixmaps_Dir : constant String := "pixmaps/";
   begin
      -- Creation of the Dialog Window
      Gtk.Dialog.Initialize (MainDialog);
      MainDialog.Set_Title (To_String(Lan_Dialog_Title(Lan)));
      ...
      Dialog_CBR.Connect(MainDialog, "delete_event",
                         Dialog_CBR.To_Marshaller(On_DNames_Delete_Event'Access));
      Button.Connect(MainDialog.OK, "clicked",
                     Button.To_Marshaller(On_DOK'Access));
      Button.Connect(MainDialog.Cancel, "clicked",
                     Button.To_Marshaller(On_DNames_End'Access));
   end Init;
